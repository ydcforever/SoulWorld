...
YDC.path = {
    name: window.document.location.pathname,
    root: function (url) {
        if (isBlank(this.name.substring(1))) {
            return window.location.protocol + "//" + window.location.host + url;
        } else {
            return window.location.protocol + "//" + window.location.host + this.name.substring(0, this.name.lastIndexOf('/')) + url;
        }
    }
...
YDC.dateOperation = {
    ...
    show : function($time) {
        var today = new this.Today(new Date());
        $time.text(today.year + '-' + today.month + '-' + today.day + " " + today.time);
    },
    keep: function($time) {
        setInterval(function(){
            YDC.dateOperation.show($time);
        }, 1000);
    }
};
...
YDC.elementBuilder = {
     ...
     * @returns {HTMLElement}
     */
    a: function (className, href, text) {
        var aElement = document.createElement('a');
        if (!isBlank(className))aElement.className = className;
        if (!isBlank(href)) aElement.href = href;
        if (!isBlank(text)) aElement.innerText = text;
        return aElement;
    },
    /**
     *
